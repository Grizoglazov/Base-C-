...
public:
	...
	void setProduct(std::string, std::string, unsigned, unsigned);
...
int start() {
	...
	for (size_t i = 0; i < count; ++i)
	{
		...
		readIn >> readA;
		readIn.ignore();
		product[i].setProduct(readN, readM, readQ, readA);
	}
...
void Production::setProduct(std::string pN, std::string pM, unsigned pQ, unsigned pA) {
...
void Production::setPrice(double P) { price = P; }

double Production::getPrice() const { return price; }
...
void Production::showProduct() const {

	std::cout << getProductName() << "  "
		<< getProductManufacturer() << "  "
		<< getProductQuantity() << "  "
		<< getProductArticle() << std::endl;
}
